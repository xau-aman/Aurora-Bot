"""Train a BPE tokenizer on Aurora's corpus."""
import os
...
SPECIAL_TOKENS = ["<|pad|>", "<|unk|>", "<|bos|>", "<|eos|>", "<|system|>", "<|user|>", "<|aurora|>", "<|end|>"]
...
def main():
    ...
    out_path = os.path.join(OUT_DIR, "aurora_tokenizer.json")
    tokenizer.save(out_path)

    # Test
    test_texts = [
        "<|system|>You are Aurora.<|end|>",
        "<|user|>What is an array?<|end|>",
        "<|aurora|>An array is a contiguous block of memory.<|end|>",
    ]
    for t in test_texts:
